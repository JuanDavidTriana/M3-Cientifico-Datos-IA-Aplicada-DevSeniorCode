"""Carga el modelo OPTIMIZADO (.tflite, podado + cuantizado) y ejecuta inferencia.

Misma lógica que proyecto_02_api_fastapi_docker/app/model_utils.py, duplicada
aquí para que este proyecto sea un servicio Docker autocontenido y comparable
directamente contra servicio_baseline/.
"""
import os
import time
...
class ModeloOptimizado:
    ...
    def _preprocesar(self, imagen: np.ndarray) -> np.ndarray:
        ...
        if imagen.max() > 1.0:
            imagen = imagen / 255.0
        return imagen.reshape(1, 28, 28, 1).astype(self.entrada["dtype"])
